#!/usr/bin/env python3
"""Import a PredskazBot v2 seed/live JSONL file into PostgreSQL.

The importer is intentionally small and idempotent: every v2 seed row must have
an entity name matching a v2 table and a data object containing that table's
columns. Rows are upserted by the `id` column when present, otherwise by the
natural unique keys used by the current v2 schema.
"""
...
def upsert_sql(table: str, row: dict[str, Any]) -> str:
    ...
    conflict_columns = ", ".join(CONFLICT_TARGETS[table])
    ...
    return f"INSERT INTO {table} ({quoted_columns}) VALUES ({placeholders}) ON CONFLICT ({conflict_columns}) {conflict_action}"
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    for entity in TABLE_ORDER:
        ...
        if count:
            print(f"- imported {entity}: {count}")
    print("v2 seed import complete")
    return 0
